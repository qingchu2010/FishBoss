...
function getPreview(tc: ToolCall): string {
  ...
  switch (tc.function.name) {
    case 'bash':
      return shortenText(getRawArg(argsText, 'command') || String(args['command'] || ''))
    case 'read':
    case 'write':
    case 'edit':
      return shortenText(getRawArg(argsText, 'filePath') || getRawArg(argsText, 'path') || String(args['filePath'] || args['path'] || ''))
    case 'grep':
    case 'glob': {
      const pattern = getRawArg(argsText, 'pattern') || String(args['pattern'] || '')
      const path = getRawArg(argsText, 'path') || getRawArg(argsText, 'filePath') || String(args['path'] || args['filePath'] || '')
      return shortenText([pattern, path].filter(Boolean).join(' '))
    }
    case 'ls':
...
function getEditAdded(tc: ToolCall): number {
  if (tc.function.name !== 'edit') return 0
  return countLines(getRawArg(tc.function.arguments, 'newString'))
}

function getEditRemoved(tc: ToolCall): number {
  if (tc.function.name !== 'edit') return 0
  return countLines(getRawArg(tc.function.arguments, 'oldString'))
}

function getWriteAdded(tc: ToolCall): number {
  if (tc.function.name !== 'write') return 0
